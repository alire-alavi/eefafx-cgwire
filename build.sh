...
source_env /home/alire/work/eefa/docker-cgwire/.env


function get_kitsu_version() {
    echo "${MAGNETA}getting kitsu version"
...
function get_zou_version(){
    echo "${MAGNETA} getting zou version"
...
function init_zou() {
    ...
    if docker compose exec db psql -U ${dbowner} ${dbname} -c '' 2>&1; then
        ...
        docker compose exec zou-app zou reset-search-index
